...
@push('scripts')
<script>
window._canDeleteTraining = {{ auth()->user()->isAdmin() ? 'true' : 'false' }};
(function() {
    const baseUrl = '{{ url("/") }}';
    ...
    function renderPagination(meta) {
        ...
        function makePageNav(id) {
            ...
            function addPage(num) {
                ...
                a.addEventListener('click', function(e) { e.preventDefault(); window._trainingsPage = num; loadTrainings(); });
                li.appendChild(a);
                ul.appendChild(li);
            }
            addPage(1);
    ...
    document.getElementById('trainings-per-page') && document.getElementById('trainings-per-page').addEventListener('change', function() {
        window._trainingsPage = 1;
        var per = document.getElementById('trainings-cards-per-page'); if (per) per.value = this.value;
        loadTrainings();
    });
    document.getElementById('trainings-cards-per-page') && document.getElementById('trainings-cards-per-page').addEventListener('change', function() {
        window._trainingsPage = 1;
        var per = document.getElementById('trainings-per-page'); if (per) per.value = this.value;
        loadTrainings();
    });
    ...
    document.body.addEventListener('click', async function(e) {
        ...
        if (editBtn) {
            ...
            } catch (err) { alert('Error loading training.'); }
            return;
        }
    ...
    document.getElementById('btn-do-assign').addEventListener('click', async function() {
        ...
        if (userIds.length === 0) { alert('Select at least one personnel.'); return; }
        try {
            ...
            loadTrainings();
        } catch (e) { alert('Error assigning personnel.'); }
    });
    ...
    document.getElementById('btn-update-training').addEventListener('click', async function() {
        ...
        try {
            ...
            loadTrainings();
        } catch (e) { alert('Error updating training.'); }
    });
    ...
    document.getElementById('btn-save-training').addEventListener('click', async function() {
        ...
        try {
            ...
            form.reset();
            loadTrainings();
        } catch (e) { alert('Error saving training.'); }
    });
    ...
    document.getElementById('btn-do-import').addEventListener('click', async function(ev) {
        ...
        if (!userIds.length) { alert('Please select at least one user.'); return; }
        ...
        } catch (e) { console.error('Import error', e); alert('Error: ' + (e.message || 'Please try again.')); }
